...
* show a timer on the presenters view - elapsed / remaining
...
* json
* GLI gem
* RMagick (optional, for resizing images)
* PDFKit (optional, for generating PDF of presentation) https://github.com/jdpace/PDFKit

To get these requirements installed on xUbuntu, use the following:

    $ sudo apt-get install ruby ruby-sinatra ruby-bluecloth ruby-nokogiri ruby-json ruby-rmagick
	$ sudo gem install gli pdfkit showoff

== Getting ShowOff installed
...
markdown files for the slides you're presenting.
    ...
    $ git clone (showoff-repo) # currently https://github.com/puppetlabs/showoff.git
    $ cd (showoff-repo) # this would be showoff
    $ showoff serve
...
In the future, ShowOff might/will/can:
