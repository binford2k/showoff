...
= Executing Ruby or shell code live

Showoff can execute Ruby or shell code from a slide and display the results
live.  To do this, mark your code block with the keyword <tt>execute</tt>.
...
must be started with the <tt>-x</tt> or <tt>--execute</tt> flags to enable code
execution.
...
If you need supporting libraries when you evaluate the code, you can put Ruby
files (*.rb) into the root directory of the presentation and they will be
required when the presentation loads.
